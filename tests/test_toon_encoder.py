...
class TestToonEncoderStreaming:
    """Tests for streaming TOON encoding."""

    def test_stream_single_item(self) -> None:
        """Test streaming with single item."""
        encoder = ToonEncoder()
        stream = iter([[{"id": 1, "name": "Alice"}]])
        result = list(encoder.encode_stream(stream))
    ...
    def test_stream_skips_non_dict(self) -> None:
        """Test streaming skips non-dict items."""
        encoder = ToonEncoder()
        stream = iter([
            [{"id": 1}],
